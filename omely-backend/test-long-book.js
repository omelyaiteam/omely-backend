// TEST LIVRE TRÈS LONG - SYSTÈME DE CHUNKS
const longBookText = `
...
${'A'.repeat(100000)} // Ajouter 100k caractères pour simuler un livre très long
`;

async function testLongBook() {
  console.log('🧪 TEST LIVRE TRÈS LONG - SYSTÈME DE CHUNKS');
  console.log('===========================================');
  console.log(`📄 Longueur du livre: ${longBookText.length} caractères`);
  
  try {
    console.log('📤 Test du endpoint /extract/book avec livre très long...');
    ...
    const response = await fetch('https://omely-node-backend.fly.dev/extract/book', {
      ...
      body: JSON.stringify({
        text: longBookText,
        bookTitle: "Les Secrets d'un Esprit Millionnaire (Version Longue)"
      })
    });
    ...
    if (result.status === 'success') {
      ...
      const checks = {
        ...
        isChunked: result.metadata?.extractionMethod === 'CHUNKED_BOOK_EXTRACTION'
      };
      ...
      if (qualityScore >= 90) {
        console.log('🎉 SUCCÈS TOTAL: Le système de chunks fonctionne parfaitement !');
        console.log('✅ Extraction complète activée');
        console.log('✅ Système de chunks opérationnel');
