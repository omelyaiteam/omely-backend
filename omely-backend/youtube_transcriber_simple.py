...
model = whisper.load_model("tiny")  # Fastest model to avoid timeouts
...
def download_audio(url, output_path):
    """Download audio from YouTube video with optimized settings"""
    ydl_opts = {
        'format': 'worstaudio/worst',  # Fastest download
        'postprocessors': [{
            'key': 'FFmpegExtractAudio',
            'preferredcodec': 'mp3',
            'preferredquality': '64',  # Lower quality = faster
        }],
        'outtmpl': output_path,
        'quiet': True,
        'no_warnings': True,
...
def health_check():
    ...
    return jsonify({'status': 'healthy', 'service': 'youtube-transcriber-simple'})

if __name__ == '__main__':
    print("Starting YouTube Transcriber Server (Simple)...")
